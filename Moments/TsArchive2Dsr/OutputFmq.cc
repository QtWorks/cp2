...

{
  ...
  if (_params.output_fields.cmd_vel) {
    _addField("CMD_VEL", "m/s", _velScale, _velBias, fp);
    _nFields++;
  }
  ...
  if (_params.output_fields.cmd_width) {
    _addField("CMD_WIDTH", "m/s", _widthScale, _widthBias, fp);
    _nFields++;
  }
  ...
  if (_params.output_fields.cmd_wx2peak_sep) {
    _addField("CMD_WX2PEAK_SEP", "fraction", _fracScale, _fracBias, fp);
    _nFields++;
  }
  
  if (_params.output_fields.cmd_cvar_db) {
    _addField("CMD_CVAR_DB", "dB", _dbmScale, _dbmBias, fp);
    _nFields++;
  }
...

{
  ...
  for (int igate = 0; igate < _nGates; igate++) {
    ...
    if (_params.output_fields.cmd_vel) {
      *dp = _convertDouble(fields.cmd_vel, _velScale, _velBias);
      dp++;
    }
    ...
    if (_params.output_fields.cmd_width) {
      *dp = _convertDouble(fields.cmd_width, _widthScale, _widthBias);
      dp++;
    }
    ...
    if (_params.output_fields.cmd_wx2peak_sep) {
      *dp = _convertDouble(fields.cmd_wx2peak_sep, _fracScale, _fracBias);
      dp++;
    }

    if (_params.output_fields.cmd_cvar_db) {
      *dp = _convertDouble(fields.cmd_cvar_db, _dbmScale, _dbmBias);
      dp++;
    }
