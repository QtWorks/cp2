...
#define PIRAQ_CLOCK_FREQ 10000000  /* 10 Mhz */
	...
	uint4 secs;        /* Unix standard - seconds since 1/1/1970 = pulse_num * N / ClockFrequency */
...
#define PX_SZ_COMMENT 64
	...
	float4 h_rconst;
	float4 v_rconst;
	float4 peak_power;          /* added by JVA -  needed for v/h_channel_radar_const */
...
/// array of structures containing SynthAngle output for all channels
struct synth { 
	float el_off_ref; 
	float az_off_ref; 
	float xmit_freq; 
